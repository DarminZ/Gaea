export default {
      ...
      methods: {
        select: function () {
          ...
          if (this.type === 'multiple') {
            var multiInx = this.model.value.indexOf(this.opt)
            if (multiInx > -1) {
              this.model.value.splice(multiInx, 1)
            } else {
  ...
  methods: {
    reset: function () {
      switch (this.type) {
        case this.typeEnum.single: this.answer.value = ''
          break
        case this.typeEnum.multiple: this.answer.value = []
          break
        case this.typeEnum.text: this.answer.value = ''
          break
      }
    },
    submit: function (evt) {
